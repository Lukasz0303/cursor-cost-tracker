...
- Coding stats: AI lines come from local composer headers (not the usage API) as composer **added + removed** (not every rewrite; `totalLinesRemoved` is included). Effectiveness is **git insertions on the default branch ÷ AI** for this repo — not `AI − pending`, and not all insertions ever on main. All on Cursor is the dashboard Lines Edited total; git bars stay this repo. Tab completions are still not in local headers.
